...
Graph::Graph(bool weighted, bool directed)
    : weighted(weighted), directed(directed), random(Random(0)) {}
...
bool Graph::assertEdgeExists(Vertex source, Vertex destination,
  ...
  if (adjacency_list[source].find(destination) ==
      adjacency_list[source].end()) {
    if (functionName != "")
      error(functionName + " called on nonexistent edge " + source + " -> " +
            destination);
    return false;
  }
  ...
  if (!directed) {
    ...
    if (adjacency_list[destination].find(source) ==
      ...
      if (functionName != "")
        error(functionName + " called on nonexistent edge " + destination +
              " -> " + source);
      return false;
    }
  }
...
void Graph::savePNG(string title) const {
  ...
  neatoFile.close();
  string command =
      "neato -n -Tpng " + filename + " -o " + title + ".png 2> /dev/null";
  int result = system(command.c_str());

  if (result == 0) {
    cout << "Output graph saved as " << title << ".png" << endl;
  } else {
    cout << "Failed to generate visual output graph using `neato`. Run `make "
            "install_graphviz` first to install dependencies."
         << endl;
  }
}
